...
(function () {
  ...
  window.getColor = function (i) {
    ...
    return colorPalette[
      Math.floor(startColorIdx + 4 * i) % colorPalette.length
    ];
  };
})();
...
function updateData() {
  ...
  $("li.item:not(.empty)").each(function (i, e) {
    ...
    $e.css(
      "background",
      "linear-gradient(to right, " + color + " 11%, rgba(0, 0, 0, 0) 0%)"
    );
  });
...
$(document).on("click", ".item-weight-add, .item-weight-sub", function (evt) {
  ...
  var sign = $this.is(".item-weight-add")
    ? +1
    : $this.is(".item-weight-sub")
    ? -1
    : 0;
  ...
  var w = Math.floor(
    Math.max(
      1,
      Math.min(9999, (parseInt($w.attr("data-weight")) || 1) + sign * amount)
    )
  );
...
function Roulette() {
  ...
  function OnStart() {
    try {
      ...
      window.radius = Math.min(
        Math.min(R.size.width, R.size.height) * 0.5 * 0.9,
        R.size.height / 2 - 40
      );
      ...
      // R.buffer.drawImage(bgImage, 100, 100, 996, 935)
  ...
  function OnUpdate(timestamp) {
    ...
    if (R.angularDamping) {
      R.angularVelocity -=
        Math.max(20, Math.min(R.angularVelocity * 0.5, R.angularDamping)) *
        (R.deltaTime / 1000);
    }
  ...
  function OnRender() {
    ...
    if (R.Items && R.Items.length > 0) {
      ...
      R.Items.forEach(function (item) {
        ...
        while (R.currentAngle >= 360) R.currentAngle -= 360;
        if (
          item.angleSum - item.angle <= R.currentAngle &&
          R.currentAngle <= item.angleSum
        ) {
          R.current = item.name;
...
(function () {
  ...
  $(document)
    .on(
      ...
      ".item-weight-sub, .item-weight-add",
      function (e) {
        move();
        show();
      }
